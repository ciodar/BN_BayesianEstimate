...
#bn.plot()
import os
files = os.listdir("resources/datasets")
for filename in sorted([filename for filename in files if 'Cases' in filename and filename != 'Cases.csv'],key = lambda f: int(f[0:f.index('Cases')])):
    if filename.endswith(".csv") and int(filename[0:filename.index('Cases')]) <= 10000:
         # print(os.path.join(directory, filename))
        bn = BayesianNet()
        bn.readNetwork("resources/cancer3uniform.bn")
        bayesEstimate(os.path.join("resources\datasets",filename),bn)
        print('KL divergence for n =',filename[0:filename.index('Cases')], calculateKLDivergency(bn))
    else:
        continue
#
# bNet10 = BayesianNet()
...
#
# bNet50 = BayesianNet()
# bNet50.readNetwork("resources/cancer3uniform.bn")
# csvfile50 = 'resources/datasets/50Cases.csv'
# bayesEstimate(csvfile50, bNet50)
# print('KL divergence for n = 50', calculateKLDivergency(bNet50))
#
# bNet100 = BayesianNet()
# bNet100.readNetwork("resources/cancer3uniform.bn")
# csvfile50 = 'resources/datasets/50Cases.csv'
# bayesEstimate(csvfile50, bNet50)
# print('KL divergence for n = 50', calculateKLDivergency(bNet50))
# #
